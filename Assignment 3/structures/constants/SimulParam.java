package structures.constants;

/**
 * Stores all the constants variables of the system.
 * @author João Brito, 68137
 */
public class SimulParam {
    
    /**
     * Number of rooms in the museum.
     */
    public static final int N_ROOMS = 5;
    
    /**
     * Number of ordinary thieves.
     */
    public static final int N_THIEVES = 6;
    
    /**
     * Thieves minimum displacement.
     */
    public static final int THIEF_MIN_MD = 2;
    
    /**
     * Thieves maximum displacement.
     */
    public static final int THIEF_MAX_MD = 6;
    
    /**
     * Museum minimum paintings.
     */
    public static final int N_MIN_PAINTINGS = 8;
    
    /**
     * Museum maximum paintings.
     */
    public static final int N_MAX_PAINTINGS = 16;
    
    /**
     * Museum minimum distance.
     */
    public static final int N_MIN_DISTANCE = 15;
    
    /**
     * Museum maximum distance.
     */
    public static final int N_MAX_DISTANCE = 30;
    
    /**
     * Assault party elements number.
     */
    public static final int N_ELEM_ASSAULT_PARTY = 3;

}
